select itm.itemcategorycode category, subcategory,
...
Left Join
    (Select * From libdata.dbo.item_master Where ean is not null and not parentStyleNo like 'test%' ) itm
    on itm.ean=soh.barcode
Left Join
    lib_backup.dbo.olabi_product_sku sk on sk.barcode = soh.barcode
where sk.vendor_name LIKE '%SACHI%'
group by itm.itemcategorycode, subcategory
order by itm.itemcategorycode
